...
public class OrderServiceImpl implements IOrderService {
    ...
    public ResponseService pay(Integer userId, Long orderNo, String path) {
        ...
        if (orderNo == null){
            return ResponseService.creatByError("参数错误");
        }
        Order order = orderMapper.setlectByUserIdAndOrderNo(userId,orderNo);
        if (order == null){
            return ResponseService.creatByError("用户没有该订单");
        }
        ...
        String subject = new StringBuilder().append("无敌大浩浩品牌扫码支付，订单号：").
        ...
        String body = new StringBuilder().append("订单").append(order.getOrderNo().toString())
                                                        .append("总共消费")
        ...
        switch (result.getTradeStatus()) {
            ...
            case FAILED:
                ...
                return ResponseService.creatByError("支付宝预下单失败!!!");

            case UNKNOWN:
                LOGGER.error("系统异常，预下单状态未知!!!");
                return ResponseService.creatByError("系统异常，预下单状态未知!!!");

            default:
                LOGGER.error("不支持的交易状态，交易返回异常!!!");
                return ResponseService.creatByError("不支持的交易状态，交易返回异常!!!");
        }
    }

    @Override
    public ResponseService aliCallback(Map<String, String> params) {
        long orderNo = Long.parseLong(params.get("out_trade_no"));
        String tradeNo = params.get("trade_no");
        String tradeStatus = params.get("trade_status");
        Order order = orderMapper.selectByOrderNo(orderNo);
        if(order == null){
            return ResponseService.creatByError("非本商城的订单,回调忽略");
        }
        if(order.getStatus() >= Const.OrderStatusEnum.PAID.getCode()){
            return ResponseService.creatBySuccess("支付宝重复调用");
        }
